...
These code examples are intended to reinforce usage of the Arm® Cortex® in PIC32CM PL10 MCU low-power modes, similar to the Microchip MASTERs 2025, 25009 MCU6 class. These exercises guide you in configuring peripherals for low power, understanding the code generated with MPLAB® Code Configurator (MCC), and using it in your application.

These examples provide specific instructions to reduce system power consumption using sleep modes, performance levels, clock domains, the Event system, and sleepwalking peripherals. Upon completion of the class, you are encouraged to use additional resources (identified in the example materials) to further your knowledge and understanding.

**<u>Upon completion, you will:</u>**
...
**NOTE:** Open VS Code, then use **File → Open Folder...** and select the lab folder (for example, PL10_LowPower_LAB1). The folder appears in the Explorer and becomes your workspace.
...
### Overview
This lab uses the PL10_LowPower_LAB1 MPLAB Harmony v3 project, configured with MPLAB Code Configurator (MCC), to demonstrate how to measure the baseline current of the PIC32CM PL10 MCU in Standby mode. The project is set up so that the MCU enters Standby mode immediately after startup, allowing you to measure its low-power consumption with no additional application functionality running.

Once the device is in Standby mode, you will use the Curiosity Nano Explorer's current measurement features to observe and record the MCU's power usage. This establishes a reference for future labs.

This baseline capture is intentionally simple: it isolates regulator behavior, clock tree settings, and standby leakage so you can compare later labs against a known low-power floor. When you review the time plot, expect a steady, low-current trace with minimal activity, since no peripherals are actively sampling or transmitting.
...
- The PAC1944 has four measurement channels:
  - CH1 (μA range, 51Ω sense resistor) and CH4 (mA range, 0.22Ω sense resistor) measure the CNANO P3V3 power domain.
...
- For accurate low-power measurements of the PL10 CNano:
  - Use CH1 for microamp (μA) measurements (for currents below 2 mA).
...
- To select the measurement channel:
  ...
  - Remove the jumper from J506 for CH1 (μA) measurements (CH1 connected).
  ...
  - For all the labs, we will be monitoring the current draw of the PIC32CM PL10 CNano on Channel 1. 
...
- After all the setup is complete, connect the Curiosity Nano Explorer to your PC via USB for measuring the PIC32CM PL10 CNano current draw.
...
1.1 Click the Explorer tab to see your options.
...
- The folder appears in the VS Code Explorer tab. This becomes your workspace and contains all `src` and `mcc` files inside `My_MCC_Config`.
...
1.5 Use `Ctrl` + `Shift` + `P` on Windows to open the Command Palette. You can also use `View > Command Palette` from the application menu.
...
- Use the Command Palette within VS Code to build the project.
  ...
  - Type in the Command Palette `MPLAB Clean Build` and you will see the command to clean and build the project.
  ...
  - Select the PIC32CM PL10 CNano connected to your computer as the debug tool. 
...
- Remove the channels you do not wish to monitor (Voltage and/or Power). For this example, we will only monitor the current draw of the PIC32CM PL10 CNano on Channel 1.
...
- In the Lab 1 baseline capture, the trace should look steady with minimal activity. In our plot, Ch1 averages about 1.16 mA (min about 1.10 mA, max about 1.23 mA) and Ch4 averages about 1.15 mA (min about 1.08 mA, max about 1.27 mA). Use this as the reference point for comparing the added bursts and higher averages in Labs 2-4.



# Lab 2
### Purpose
After completing Lab 2, you will understand:
...
Compared to Lab 1, this lab adds an active sampling and UART reporting workload. In the time plot you should see repetitive current pulses from powering the sensor and running the ADC, with occasional UART bursts when the brightness threshold is exceeded. The optional CPU clock scaling step highlights how reducing core frequency during idle periods can lower the average without changing the sampling rate.
...
- Configure the Curiosity Nano Explorer board to measure the MCU currents as in Lab 1.
  ...
  - After setup, connect the PIC32CM PL10 Curiosity Nano and the Curiosity Nano Explorer board to your computer via the USB cables.
## Lab 2 Structure
...
- Configure the SUPC Block:
  - By default, the Standby mode operation will automatically switch from the LDO to the ULP regulator since we will be running on the 32.768 kHz oscillator.
...
- **Optional (commented out by default):** Restore CPU to full speed (8 MHz) for active sampling by calling `SetCPUSpeedToNormal()`. Uncomment to enable dynamic clock speed management for additional power savings. You must uncomment this section and the second section at the end of the `while` loop.
...
- Under `Connections > CNano Explorer`, toggle the slider to connect to the device.
- Once connected, click the graph icon to display raw data on the time plot. This will enable all the Power Monitor channels and they can be seen in the Data Axis panel.
- For this example, we will monitor the current draw of the PIC32CM PL10 CNano on Ch1 and Ch4. As seen below, the Data Visualizer shows that the maximum current for Ch1 is around 2mA, which corresponds with the Curiosity Nano Explorer User Guide. We can then see the Min, Max, and Average of the current on Ch4. The peaks of these pulses are from the AMBIENT Click Board being powered by the PIC32CM PL10 CNano.
...
#### Once the AMBIENT Click Board senses that there is a bright environment, the terminal shows the message "Bright!" along with last 20 light samples:
  ...
  - In the active-mode capture (no CPU scaling), the plot shows a higher average with regular current bursts from sensor power and ADC sampling. Our example shows Ch1 averaging about 1.96 mA with a peak around 5.03 mA, reflecting the added workload relative to the Lab 1 baseline.

  ![Lab 2 Power Analysis](./images/lab2_power_analysis_no_cpu.png)

  - With dynamic CPU speed adjustments enabled, the average drops while the peaks stay similar. In our capture, Ch1 averages about 1.31 mA and Ch4 averages about 1.64 mA, with a peak around 5.1 mA. This shows the same sampling workload with lower idle power.

  ![Lab 2 Power Analysis](./images/lab2_power_analysis_with_cpu.png)
  
# Lab 3
### Purpose
After completing Lab 3, you will understand:
...
- Configure the Curiosity Nano Explorer board to measure the MCU currents as in Lab 1 and Lab 2.
  ...
  - After setup, connect both the PIC32CM PL10 Curiosity Nano and the Curiosity Nano Explorer board to your computer via the USB cables.
...
- Develop Lab 3 by extending Lab 2. You can also open the Lab 3 project in VS Code and launch MPLAB Harmony Code Configurator (MCC).
- Close the MPLAB Data Visualizer before launching MCC.
...
- Configure the SUPC (Supply Controller) Block for Ultra-Low-Power Operation:
  - As in Lab 2, the **Standby mode operation** will automatically switch from the LDO to the ULP regulator since we will be running on the 32.768 kHz oscillator.
...
#### When the AMBIENT Click Board senses a bright environment, the terminal shows the message "Bright!" along with the last 20 light samples. When the light sensor is in a dark environment, no data is displayed on the terminal:
  ...
  - In the Lab 3 standby capture, the average current is lower than Lab 2 and the bursts are shorter, aligning with the standby sleep between phases. Our plot shows Ch1 averaging about 895 uA (min about 694 uA, max about 1.96 mA) and Ch4 averaging about 1.06 mA with a peak around 3.52 mA. This confirms the reduced idle power while still showing brief sampling and transmit activity.
...
After completing Lab 4, you will understand:
...
- Open the **Event Configurator** inside the **Plugins** dropdown menu inside MCC to set the channels and events we will use to work in standby. This will verify Event Status and User Ready are working.
...
**Note:** For each channel, ensure you have enabled Clock on Demand and Run in Standby for each peripheral (see each peripheral configuration above).
